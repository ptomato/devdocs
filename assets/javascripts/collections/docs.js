...
(function() {
  ...
  const Cls = (app.collections.Docs = class Docs extends app.Collection {
    ...
    clearCache() {
      for (let doc of Array.from(this.models)) { doc.clearCache(); }
    }
    ...
    checkForUpdates(callback) {
      this.getInstallStatuses(statuses => {
        let i = 0;
        if (statuses) {
          for (let slug in statuses) { const status = statuses[slug]; if (this.findBy('slug', slug).isOutdated(status)) { i += 1; } }
        }
        callback(i);
      });
    }
